...
def pointDistribution(listofvalues,minvalue=None, maxvalue=None,title="Points", plot = None):
    ...
    plot1.scatter(listofvalues, np.zeros_like(listofvalues), 'x')
    plot1.set_title(title)
    ...
    plot1.set_xlim([minvalue,maxvalue])
...
def ProjectionPlotRADEC(list1=None,list2 = True,list3=False,title='Map'):
...
def AmericaMapScatter(xs,ys,colors=None,title='Map'):
    ...
    #convert to map scaling from lat,lngs
    mXs = []
    mYs = []
    for index in range(len(xs)):
        mx,my = m(xs[index],ys[index])
        mXs.append(mx)
        mYs.append(my)
        
    m.scatter(mXs,mYs,c=colors,s=10,cmap = cm.gist_rainbow)
    plt.show()
    

def ScatterPlot(xs, ys, colors = None, errors = None, Projection = None, xlabel='X',ylabel='Y',title='Map'):
    ...
    fig = plt.figure()
    plot1 = fig.add_subplot(111,projection=Projection)
    pairs = plot1.scatter(xs,ys,c=colors,s=10,cmap = cm.gist_rainbow)
    ...
    if type(errors) is list:
        for index in range(len(xs)):
            xerr = errors[index][0] if type (errors[index]) is tuple else 0
            yerr = errors[index][1] if type (errors[index]) is tuple else errors[index]
            plt.errorbar(xs[index], ys[index], xerr=xerr, yerr=yerr)
    plot1.grid(True)
    
    #lnd =  plot1.legend([pairs],[''])
    #lnd.draggable(True)
